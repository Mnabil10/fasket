...
async function bootstrap() {
    ...
    const parseOrigins = (raw) => (raw || '')
        ...
        .filter(Boolean);
    const envOrigins = [
        ...parseOrigins(configService.get('ALLOWED_ORIGINS')),
        ...parseOrigins(configService.get('CORS_ALLOWED_ORIGINS')),
        ...parseOrigins(configService.get('CORS_DEV_ORIGINS')),
    ];
    const allowLocalhost = (configService.get('CORS_ALLOW_LOCALHOST') ?? 'false') === 'true';
    const exactOrigins = new Set([...defaultOrigins, ...envOrigins]
    ...
    const regexOrigins = envOrigins
        ...
        try {
            ...
            logger.warn(`Invalid CORS regex "${pattern}": ${err.message}`);
            return null;
        }
    })
        .filter((v) => Boolean(v));
    ...
    app.enableCors({
        origin(origin, callback) {
            ...
            if (exactOrigins.has(normalized))
                return callback(null, true);
            if (allowLocalhost && isLocalhost(normalized))
                return callback(null, true);
            if (regexOrigins.some((rx) => rx.test(normalized)))
                return callback(null, true);
            const withoutPort = normalized.replace(/:\d+$/, '');
            if (exactOrigins.has(withoutPort))
                return callback(null, true);
            if (allowLocalhost && isLocalhost(withoutPort))
                return callback(null, true);
            if (regexOrigins.some((rx) => rx.test(withoutPort)))
                return callback(null, true);
