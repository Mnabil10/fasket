...
let LoyaltyService = class LoyaltyService {
    ...
    async redeemPoints(params) {
        ...
        if (!config.enabled) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_DISABLED, 'Loyalty program is not enabled');
        }
        if (config.redeemRate <= 0 || config.redeemUnitCents <= 0 || config.redeemRateValue <= 0) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_RULE_VIOLATION, 'Redeem rules are not configured');
        }
        const requestedPoints = Math.floor(params.pointsToRedeem);
        if (requestedPoints < config.minRedeemPoints) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_RULE_VIOLATION, `You need at least ${config.minRedeemPoints} points to redeem`);
        }
        if (config.maxRedeemPerOrder && requestedPoints > config.maxRedeemPerOrder) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_RULE_VIOLATION, `You can redeem up to ${config.maxRedeemPerOrder} points per order`);
        }
        ...
        if (!user || user.loyaltyPoints < requestedPoints) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_NOT_ENOUGH_POINTS, 'Not enough loyalty points to redeem');
        }
        ...
        if (discountCents <= 0) {
            throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_RULE_VIOLATION, 'Not enough points to redeem a reward');
        }
        const maxDiscountFromPercent = config.maxDiscountPercent > 0
    ...
    async getUserSummary(userId, options) {
        ...
        if (!user) {
            throw new errors_1.DomainError(errors_1.ErrorCode.USER_NOT_FOUND, 'User not found');
        }
    ...
    async getAdminSummary(userId, options) {
    ...
    async adjustUserPoints(params) {
        ...
        if (!Number.isFinite(points) || points === 0) {
            throw new errors_1.DomainError(errors_1.ErrorCode.VALIDATION_FAILED, 'Points adjustment must be a non-zero integer');
        }
        if (!params.reason?.trim()) {
            throw new errors_1.DomainError(errors_1.ErrorCode.VALIDATION_FAILED, 'Adjustment reason is required');
        }
        const { user, transaction } = await this.prisma.$transaction(async (tx) => {
            ...
            if (nextBalance < 0) {
                throw new errors_1.DomainError(errors_1.ErrorCode.LOYALTY_RULE_VIOLATION, 'Adjustment would reduce loyalty balance below zero');
            }
    ...
    async ensureCycle(userId, threshold, tx) {
